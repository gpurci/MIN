...
class GroupKP(object):
    ...
    def __call__(self, map_distance:"np.array"):
        self.groups[-1].setMap(map_distance)
        ...
        for generation in range(GroupTSP.GENERATIONS):
            ...
            population = self.groups[-1].setExtreme(population, migrants)
    ...
    def migration(self, best_distance):
        ...
        if (np.allclose(check_distance,  best_distance, rtol=1e-02, atol=1e-08)):
            ...
            for i in range(1, self.nbr_groups-1):
                ...
                population = self.groups[i].setExtreme(population, migrant)
            else:
                population = self.populations[0]
                migrant    = self.best_individs[-2].reshape(1, -1)
                population = self.groups[0].setExtreme(population, migrant)

            distances = []
            for i in range(0, self.nbr_groups-1):
                print("Check weak group")
                best_individ = self.best_individs[i-1]
                distance     = self.groups[i].getIndividDistance(best_individ)
                distances.append(distance)
            distances = np.array(distances, dtype=np.float32)
            arg_min = np.argmin(distances).reshape(-1)[0]
