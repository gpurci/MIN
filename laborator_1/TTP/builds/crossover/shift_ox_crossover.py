...
class CrossoverOXShift(CrossoverBase):
    """
    Clasa 'CrossoverOXShift', 
    Functia 'crossover' are 2 parametri, parinte1, parinte2.
    Metoda 'call', returneaza functia din configuratie.
    Pentru o configuratie inexistenta, vei primi un mesaj de eroare.
    """
    def __init__(self, method, **configs):
        super().__init__(method, name="CrossoverOXShift", **configs)
    ...
    def __shift(self, parent1, parent2):
        # check diff
        mask_sim_locus = parent1==parent2
        if (mask_sim_locus.sum() == mask_sim_locus.shape[0]):
            size_shift = np.random.randint(low=self.GENOME_LENGTH//4, high=self.GENOME_LENGTH//2, size=None)
    ...
    def crossoverIn(self, parent1, parent2):
        ...
        parent2 - individ
        """
        parent2 = self.__shift(parent1, parent2)
