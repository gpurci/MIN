...
Parent: {}""".format(self.__method, self.__configs, super().__str__())
    ...
    def computeBestKscore(self, city, window_size, visited_city):
        ...
        profits   = self.__dataset["item_profit"][args]
        weights   = self.__dataset["item_weight"][args]
        scores    = profits**2/weights*distances
        count = 0
        ret_args = []
        ret_pick = []
    ...
    def recFill(self, city, window_size, visited_city, deep):
        ...
        if (self.__population_size <= 0):
            return None
        args = self.computeBestKscore(city, window_size, visited_city)
